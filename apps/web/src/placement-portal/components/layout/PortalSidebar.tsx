...
import { navAdmin, navRecruiter, navStudent, PRIMARY } from "@/placement-portal/constants";
  ...
  return (
    <div className="sb flex flex-col h-full transition-all duration-300" style={{ width: collapsed ? 64 : 240 }}>
      <div className="flex items-center gap-3 px-4 py-5 border-b" style={{ borderColor: "rgba(255,255,255,.08)" }}>
        ...
        {!collapsed && (
          <span className="font-700 text-white" style={{ fontWeight: 700, fontSize: 15 }}>
            PlaceMe
          </span>
        )}
          ...
          return (
            ...
            >
              ...
              {!collapsed && id === "notifications" && (
                <span className="ml-auto text-white rounded-full px-1.5 text-xs" style={{ background: "#ef4444", fontSize: 10 }}>
                  3
                </span>
              )}
            </button>
          ...
          <div className="flex gap-1">
            {(["student", "recruiter", "admin"] as const).map((r) => (
              <button
                ...
                style={{
                  background: role === r ? PRIMARY : "rgba(255,255,255,.08)",
      ...
      <button
        className="flex items-center gap-3 px-4 py-4 text-gray-500 hover:text-red-400 transition-colors border-t"
