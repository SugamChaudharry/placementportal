...
import { INDIGO_SHADES } from "@/placement-portal/constants";

export function RecruiterDashboard({ setPage }: { setPage: (p: string) => void }) {
  const drives = [
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            {drives.map((d, i) => (
              <Card key={i} className="p-4 hl cursor-pointer" onClick={() => setPage("rec-drives")}>
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <ColorBdg label={d.status} color={d.status === "Active" ? "#10b981" : "#f59e0b"} />
                  <span className="ml-auto text-xs text-gray-500">{d.next}</span>
                </div>
                <div className="flex items-center gap-4 mb-2">
                  <div>
                    <p className="text-lg font-700" style={{ fontWeight: 700 }}>
                      {d.applicants}
                    </p>
        ...
        <div className="space-y-4">
          <Card className="p-4">
            ...
            <div className="space-y-3">
              {[
                ["Arjun Kumar", "10:00 AM", "#4285F4"],
                ["Priya Sharma", "11:30 AM", "#7c3aed"],
                ["Rahul Singh", "2:00 PM", "#0891b2"],
              ].map(([name, time, color]) => (
                <div key={name} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-lg cursor-pointer">
                  <Av name={name} size={32} color={color} />
