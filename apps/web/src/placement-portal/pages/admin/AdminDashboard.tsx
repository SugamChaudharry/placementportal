...
import { Activity, ArrowRight, Award, Bell, Briefcase, Building2, Code2, GraduationCap, Plus, Users } from "lucide-react";
import { Av, Btn, Card, ColorBdg, SectionHeader, StatCard } from "@/placement-portal/components/atoms";
        ...
        <div className="lg:col-span-2">
          ...
          <Card>
            {[
              ...
              ["Rahul Gupta", "ME · BITS Pilani", "Student", "5h ago", "#f59e0b"],
            ].map(([name, sub, role, time, c]) => (
              <div key={name} className="flex items-center gap-3 px-5 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors">
                <Av name={name} size={36} color={c} />
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-800">{name}</p>
                  <p className="text-xs text-gray-400">{sub}</p>
                </div>
                <ColorBdg label={role} color={role === "Student" ? "#4f46e5" : "#0891b2"} />
            ...
            <div className="space-y-3">
              {health.map((h) => (
                <div key={h.label} className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-medium" style={{ color: h.status === "healthy" ? "#10b981" : "#f59e0b" }}>
                    {h.val}
                  </span>
                </div>
              ))}
          ...
          <Card className="p-5">
            ...
            {(
              [
                ...
                ["Add Company", Building2, "#0891b2", "admin-companies"],
