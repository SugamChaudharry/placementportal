...
import { FileText, Download, Building2, TrendingUp, Award } from "lucide-react";
...
import {
  ...
  XAxis, YAxis, Tooltip, ResponsiveContainer, Cell, CartesianGrid, AreaChart, Area, PieChart, Pie, Legend
} from "recharts";
import { P, chartPlacement, chartBranch, chartRadar, INDIGO_SHADES, chartDriveTrend, chartCompanyMix, topStudentsPerf } from "@/lib/constants";
    ...
    <div className="su">
      <div className="flex gap-0 border-b border-gray-200 mb-5 overflow-x-auto flex-wrap">
        {tabs.map(({ id, label }) => (
          <button key={id} onClick={() => setTab(id)}
            ...
            style={{ borderColor: tab === id ? P : "transparent", color: tab === id ? P : "#6b7280" }}>
            {label}
          </button>
        ))}
        <div className="ml-auto flex items-center gap-2 pb-2">
          ...
          <Button size="sm" icon={FileText}>NAAC Report</Button>
        </div>
      </div>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="p-5 text-center" style={{ background: "linear-gradient(135deg,#eef2ff,#fdf4ff)" }}>
              <p className="text-4xl font-700" style={{ fontWeight: 700, color: P }}>89%</p>
            ...
            <Card className="p-5">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={chartPlacement}>
                  ...
                  <Area dataKey="offers" fill="url(#indigo)" stroke={P} strokeWidth={2} />
                </AreaChart>
              </ResponsiveContainer>
            </Card>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={chartBranch} layout="vertical">
                  ...
                  <Bar dataKey="rate" radius={[0, 6, 6, 0]}>
                    {chartBranch.map((_, i) => <Cell key={i} fill={INDIGO_SHADES[Math.min(i, 4)]} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </Card>
          ...
          <Card className="mb-5 p-5">
            ...
            <ResponsiveContainer width="100%" height={240}>
              <LineChart data={chartDriveTrend}>
                ...
                <Line type="monotone" dataKey="offers" stroke={P} strokeWidth={2} name="Offers" />
                <Legend />
              </LineChart>
            </ResponsiveContainer>
          </Card>
          <Card>
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                ].map(([comp, drive, app, shortlist, offered, rate, col], idx) => (
                  <tr key={idx} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <LogoCircle letter={(comp as string)[0]} color={col as string} size={28} />
                    ...
                    <td className="py-3 px-4 font-medium" style={{ color: "#059669" }}>{offered}</td>
                    <td className="py-3 px-4"><ColorBadge label={rate as string} color="#059669" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <Card className="p-5">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <RadarChart data={chartRadar} cx="50%" cy="50%" outerRadius="75%">
                ...
                <Radar name="Avg" dataKey="A" stroke={P} fill={P} fillOpacity={0.35} />
                <Tooltip />
              </RadarChart>
            </ResponsiveContainer>
          </Card>
              ...
              <tbody>
                {topStudentsPerf.map(s => (
                  <tr key={s.name} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="py-3 px-4 font-medium" style={{ color: s.cgpa >= 9 ? "#10b981" : "#374151" }}>{s.cgpa}</td>
      ...
      {tab === "company-insights" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie data={chartCompanyMix} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} label>
                  {chartCompanyMix.map((_, i) => <Cell key={i} fill={INDIGO_SHADES[i % INDIGO_SHADES.length]} />)}
                </Pie>
                <Tooltip />
                <Legend />
              </PieChart>
          ...
          <Card className="p-5">
            <p className="font-600 text-sm mb-4" style={{ fontWeight: 600 }}>Returning recruiters</p>
            <div className="space-y-3">
              {[
                ...
                <div key={idx} className="p-3 rounded-xl border border-gray-100">
                  <p className="text-sm font-medium text-gray-800">{c}</p>
                  <p className="text-xs text-gray-500">{y}</p>
                  <p className="text-xs mt-1" style={{ color: P }}>{n}</p>
                </div>
              ))}
            </div>
          </Card>
