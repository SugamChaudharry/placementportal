...
import { Search, Upload, Download, Plus, Eye, Edit3, Lock, Unlock, Trash2 } from "lucide-react";
    ...
    <div className="su">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <Input placeholder="Search..." prefix={Search} className="w-48" />
          <Button variant="secondary" size="sm" icon={Upload}>Bulk Import CSV</Button>
          ...
          <tbody>
            {students.map(s => (
              <tr key={s.roll} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td className="py-3 px-4 font-medium" style={{ color: s.cgpa >= 8.5 ? "#10b981" : s.cgpa >= 7.5 ? "#f59e0b" : "#ef4444" }}>{s.cgpa}</td>
                <td className="py-3 px-4 text-gray-600">{s.batch}</td>
                <td className="py-3 px-4 text-gray-500">{s.registered}</td>
                <td className="py-3 px-4">
                  <span className="px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: s.status === "Active" ? "#d1fae5" : "#fee2e2", color: s.status === "Active" ? "#065f46" : "#991b1b" }}>{s.status}</span>
                </td>
                <td className="py-3 px-4">
                  <div className="flex gap-1">
                    <Button size="sm" variant="ghost" icon={Eye}>View</Button>
                    <Button size="sm" variant="ghost" icon={Edit3} />
                    <Button size="sm" variant="ghost" icon={s.status === "Active" ? Lock : Unlock} />
