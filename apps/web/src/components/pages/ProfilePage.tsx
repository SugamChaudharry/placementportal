...
// PROFILE
// ═══════════════════════════════════════════════════════════════════════════════
export default function ProfilePage({ setPage }: { setPage: (page: string) => void }) {
  const [tab, setTab] = useState("personal");
  const [editMode, setEditMode] = useState(false);
  const tabs = ["personal", "skills", "resume", "preferences", "activity"];
  const skills = { Languages: ["Python", "JavaScript", "Java", "C++"], Frameworks: ["React", "Node.js", "FastAPI", "Spring"], Tools: ["Git", "Docker", "Redis", "PostgreSQL"], Cloud: ["AWS", "GCP"] };
        ...
        <Card className="p-6 text-center">
          ...
          <div className="flex justify-center gap-4 mt-4">
            <div className="text-center">
              <p className="font-700 text-lg" style={{ fontWeight: 700 }}>8.9</p>
              <p className="text-xs text-gray-400">CGPA</p>
            </div>
            <div className="w-px bg-gray-200" />
            <div className="text-center">
              <p className="font-700 text-lg" style={{ fontWeight: 700 }}>6</p>
              <p className="text-xs text-gray-400">Applications</p>
            </div>
            <div className="w-px bg-gray-200" />
            <div className="text-center">
              <p className="font-700 text-lg" style={{ fontWeight: 700 }}>1</p>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-600" style={{ fontWeight: 600, color: P }}>74%</span>
            </div>
          ...
          <div className="p-6 fi">
            {tab === "personal" && (
              <div className="grid grid-cols-2 gap-5">
                {[["Full Name","Arjun Kumar"],["Roll Number","20CS101"],["Branch","Computer Science"],["CGPA","8.9 / 10"],["Graduation Year","2025"],["Phone","+91 98765 43210"],["Date of Birth","15 Aug 2002"],["Gender","Male"],["10th Percentage","94.2% (CBSE)"],["12th Percentage","91.6% (CBSE)"],["Active Backlogs","None"],["LinkedIn","linkedin.com/in/arjun"]].map(([k, v]) => (
                  <div key={k}>
              ...
              <div className="space-y-5">
                {Object.entries(skills).map(([cat, items]) => (
                  <div key={cat}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {items.map(s => <Tag key={s} label={s} onRemove={editMode ? () => {} : undefined} />)}
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {[{ role: "SDE Intern", company: "Groww", period: "May 2024 – Aug 2024", desc: "Built real-time portfolio tracking features using WebSocket and Redis pub/sub." },
                      { role: "Research Intern", company: "TIFR", period: "Dec 2023 – Feb 2024", desc: "Worked on NLP research for low-resource language translation." }].map((e, i) => (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {["SDE", "Backend Engineer", "ML Engineer"].map(r => <Tag key={r} label={r} />)}
                      </div>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 mb-2">Preferred Locations</p>
                      <div className="flex flex-wrap gap-2">
                        {["Bangalore", "Mumbai", "Remote"].map(l => <Tag key={l} label={l} />)}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="ml-auto flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: "#d1fae5" }}>
                    <span className="font-700 text-emerald-700" style={{ fontWeight: 700, fontSize: 24 }}>82</span>
                    <div>
