...
import { Mic, MicOff, Video, VideoOff, ScreenShare, MessageSquare, PhoneOff, CheckCircle, XCircle, BookOpen } from "lucide-react";
...
const Av = ({ name, size = 40, color = P }: { name: string; size?: number; color?: string }) => (
  <div className="rounded-full flex items-center justify-center text-white font-medium text-sm" style={{ width: size, height: size, background: color }}>
...
function MeetingsPage() {
  ...
  const meetings = [
    { id: 1, company: "Google", logo: "G", color: "#4285F4", title: "SDE Technical Interview – Round 1", type: "Technical", date: "Feb 5, 2025", time: "10:00 AM", duration: "60 min", interviewer: "Priya Nair · Senior SDE", slot: "Slot 3 of 8", status: "Scheduled" },
    { id: 2, company: "Flipkart", logo: "F", color: "#2874F0", title: "Data Analyst HR Discussion", type: "HR", date: "Feb 12, 2025", time: "2:30 PM", duration: "30 min", interviewer: "Raj Verma · HR Manager", slot: "Slot 1 of 5", status: "Scheduled" },
  ];
      ...
      <div className="fixed inset-0 bg-gray-950 flex flex-col z-50 fi">
        <div className="flex-1 grid grid-cols-2 gap-3 p-4">
          ...
          <div className="bg-gray-900 rounded-2xl flex items-center justify-center relative">
            ...
            {!mic && <div className="absolute top-3 right-3 bg-red-600 rounded-full p-1"><MicOff size={14} className="text-white" /></div>}
          </div>
        </div>
        <div className="flex items-center justify-center gap-4 py-5 bg-gray-900">
          ...
          <button className="w-12 h-12 rounded-full bg-gray-700 flex items-center justify-center"><ScreenShare size={20} className="text-white" /></button>
          <button className="w-12 h-12 rounded-full bg-gray-700 flex items-center justify-center"><MessageSquare size={20} className="text-white" /></button>
          ...
          <Card key={m.id} className="p-5">
            <div className="flex items-start gap-3 mb-4">
              ...
              <div className="flex-1">
                <p className="font-600" style={{ fontWeight: 600 }}>{m.title}</p>
                <p className="text-xs text-gray-500">{m.company} · {m.type} Interview</p>
                <ColorBdg label={m.status} color="#4f46e5" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-4">
              {[["Date", m.date], ["Time", m.time], ["Duration", m.duration], ["Slot", m.slot]].map(([k, v]) => (
                <div key={k}>
            ...
            <div className="mb-4 space-y-1.5">
              {([["Camera", true], ["Microphone", true], ["Internet Speed", true]] as [string, boolean][]).map(([label, ok]) => (
                <div key={label} className="flex items-center gap-2">
                  {ok ? <CheckCircle size={14} className="text-emerald-500" /> : <XCircle size={14} className="text-red-500" />}
                  <span className="text-xs text-gray-600">{label} — {ok ? "Ready" : "Check required"}</span>
                </div>
              ))}
            </div>
            <div className="flex gap-2">
              <Btn onClick={() => setInRoom(true)} className="flex-1 justify-center" icon={Video}>Join Meeting</Btn>
              <Btn variant="secondary" icon={BookOpen}>Prepare</Btn>
            </div>
          </Card>
        ))}
      </div>
